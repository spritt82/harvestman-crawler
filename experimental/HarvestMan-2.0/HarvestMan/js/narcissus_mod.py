...
Note: This is alpha code! It does not even work correctly
for all Javascript source files which Rbnarcissus can parse.
...
#             print a[x]
    ...
    while (t.token() and t.token().type != consts["END"]):
        tt = t.get()
        print 'TT ==>',tt
...
#             print 'HERE'
            if not t.scanOperand:
                break
            operands.append(FunctionDefinition(t, x, False, consts["EXPRESSED_FORM"]))
            t.scanOperand = False
